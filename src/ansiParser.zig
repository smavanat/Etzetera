...
pub const ansi_parser = struct {
    ...
    alt_buf: *tb.text_buffer,
    main_buf: *tb.text_buffer,
    bytes: [1024]u8,

    pub fn init(texb: *tb.text_buffer, altb: *tb.text_buffer) ansi_parser {
        return ansi_parser{.state = parser_state.NORMAL, .text_buf = texb, .alt_buf = altb, .main_buf = texb, .bytes = undefined};
    }
                ...
                for(self.bytes[0..@intCast(n)]) |b| {
                    ...
                    if(b != 0) {
                        if(self.state == parser_state.NORMAL) {
                            switch(b) {
                                @intFromEnum(c0_controls.BS)  => {_=try self.main_buf.deleteText(gpa);},
                                @intFromEnum(c0_controls.HT)  => {
                                    const nextX = self.main_buf.getScreenCursorX() + (4 - (self.main_buf.getScreenCursorX() % 4));
                                    try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY(), nextX, gpa);
                                },
                                @intFromEnum(c0_controls.LF)  => {try self.main_buf.createNewLine(gpa);},
                                @intFromEnum(c0_controls.VT)  => {try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY() + 4, self.main_buf.getScreenCursorX(), gpa);},
                                @intFromEnum(c0_controls.FF)  => {try self.main_buf.clearScreen();},
                                @intFromEnum(c0_controls.CR)  => {try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY(), 0, gpa);},
                                @intFromEnum(c0_controls.ESC) => {self.state = parser_state.ESCAPE;},
                                else => {_=try self.main_buf.overwriteText(b, gpa);}
                            }
                        }
                        else if(self.state == parser_state.ESCAPE) {
                            switch (b) {
                                @intFromEnum(escape_sequences.SGR7)=> {self.main_buf.saveCursorPos();},
                                @intFromEnum(escape_sequences.SGR8)=> {self.main_buf.loadCursorPos();},
                                @intFromEnum(escape_sequences.IND) => {
                                    const newY: i32 = @max(0, @as(i32, @intCast(self.main_buf.getScreenCursorY())) + 1);
                                    try self.main_buf.screenToLogical(@intCast(newY), self.main_buf.getScreenCursorX(), gpa);
                                },
                            ...
                            if(0x40 <= b and b <= 0x7E) {
                                switch(b) {
                                    ...
                                    'H' => {
                                        if(args.items.len == 0) {
                                            try self.main_buf.screenToLogical(0, 0, gpa);
                                            args.clearRetainingCapacity();
                                        }
                                        if(args.items.len >= 2) {
                                            try self.main_buf.screenToLogical(args.items[args.items.len-2], args.items[args.items.len-1], gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'A' => {
                                        if(args.items.len >= 1) {
                                            const newY: i32 = @max(0, @as(i32, @intCast(self.main_buf.getScreenCursorY())) - @as(i32, @intCast(args.items[args.items.len-1])));
                                            try self.main_buf.screenToLogical(@intCast(newY), self.main_buf.getScreenCursorX(), gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'B' => {
                                        if(args.items.len >= 1) {
                                            const newY: i32 = @as(i32, @intCast(self.main_buf.getScreenCursorY())) + @as(i32, @intCast(args.items[args.items.len-1]));
                                            try self.main_buf.screenToLogical(@intCast(newY), self.main_buf.getScreenCursorX(), gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'C' => {
                                        if(args.items.len >= 1) {
                                            const newX: i32 = @as(i32, @intCast(self.main_buf.getScreenCursorX())) + @as(i32, @intCast(args.items[args.items.len-1]));
                                            try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY(), @intCast(newX), gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'D' => {
                                        if(args.items.len >= 1) {
                                            const newX: i32 = @max(0, @as(i32, @intCast(self.main_buf.getScreenCursorX())) - @as(i32, @intCast(args.items[args.items.len-1])));
                                            try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY(), @intCast(newX), gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'E' => {
                                        if(args.items.len >= 1) {
                                            const newY: i32 = @as(i32, @intCast(self.main_buf.getScreenCursorY())) + @as(i32, @intCast(args.items[args.items.len-1]));
                                            try self.main_buf.screenToLogical(@intCast(newY), 0, gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    ...
                                    'G' => {
                                        if(args.items.len >= 1) {
                                            try self.main_buf.screenToLogical(self.main_buf.getScreenCursorY(), args.items[args.items.len-1], gpa);
                                            args.clearRetainingCapacity();
                                        }
                                    },
                                    //ESC[s: saves the cursor position
                                    's' => {
                                        self.main_buf.saveCursorPos();
                                    },
                                    //ESC[u: loads the saved cursor position
                                    'u' => {
                                        self.main_buf.loadCursorPos();
                                    },
                                    ...
                                    'n' => {
                                        if(args.items.len > 0 and args.items[args.items.len-1] == 6) {
                                            ...
                                            const msg = try std.fmt.bufPrint(&buf, "\x1b[{d};{d}R", .{self.main_buf.getScreenCursorY()+1, self.main_buf.getScreenCursorX()+1});
                                            _=pts.write(msg);
                                        }
                                    },
                                    ...
                                    'J' => {
                                        ...
                                        if(args.items.len == 0 or args.items[args.items.len-1] == 0) {
                                            try self.main_buf.eraseText(.{.x=self.main_buf.getScreenCursorX(), .y=self.main_buf.getScreenCursorY()}, .{.x=self.main_buf.width-1, .y=self.main_buf.height-1});
                                            // std.debug.print("ESC[J\n", .{});
                                        }
                                        //ESC[1J: erase from cursor to beginning of screen
                                        else if(args.items[args.items.len-1] == 1) {
                                            try self.main_buf.eraseText(.{.x=0, .y=0}, .{.x=self.main_buf.getScreenCursorX(), .y=self.main_buf.getScreenCursorY()}, );
                                            // std.debug.print("ESC[1J\n", .{});
                                        }
                                        //ESC[2J: erase entire screen
                                        else if(args.items[args.items.len-1] == 2) {
                                            try self.main_buf.clearScreen();
                                            // std.debug.print("ESC[2J\n", .{});
                                        }
                                        //ESC[3J: erase saved lines
                                        else if(args.items[args.items.len-1] == 3) {
                                            try self.main_buf.clearScrollback(gpa);
                                    ...
                                    'K' => {
                                        ...
                                        if(args.items.len == 0 or args.items[args.items.len-1] == 0) {
                                            try self.main_buf.eraseText(.{.x=self.main_buf.getScreenCursorX(), .y=self.main_buf.getScreenCursorY()}, .{.x=self.main_buf.width-1, .y=self.main_buf.getScreenCursorY()});
                                            // std.debug.print("ESC[K\n", .{});
                                        }
                                        //ESC[1K: erase start line to cursor
                                        else if(args.items[args.items.len-1] == 1) {
                                            try self.main_buf.eraseText(.{.x=0, .y=self.main_buf.getScreenCursorY()}, .{.x=self.main_buf.getScreenCursorX(), .y=self.main_buf.getScreenCursorY()}, );
                                            // std.debug.print("ESC[1K\n", .{});
                                        }
                                        //ESC[2K: erase entire line
                                        else if(args.items[args.items.len-1] == 2) {
                                            try self.main_buf.eraseText(.{.x=0, .y=self.main_buf.getScreenCursorY()}, .{.x=self.main_buf.width-1, .y=self.main_buf.getScreenCursorY()}, );
                                    ...
                                    'm' => {
                                        ...
                                        for(args.items[0..args.items.len]) |i| {
                                            // std.debug.print("{}\n", .{i});
                                            if(sp_bg == 5) {
                                                self.main_buf.backgroundColour = c_256(@intCast(i));
                                                sp_bg = 0;
                                                continue;
                                            }
                                            if(sp_fg == 5) {
                                                self.main_buf.foregroundColour = c_256(@intCast(i));
                                                sp_fg = 0;
                                                continue;
                                            }
                                            ...
                                            switch(i) {
                                                ...
                                                0 => {
                                                    self.main_buf.currentBackgroundColour = self.main_buf.backgroundColour;
                                                    self.main_buf.currentForegroundColour = self.main_buf.foregroundColour;
                                                    is_bold = false;
                                                },
                                                ...
                                                30 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[0] else tb.basic_colours[8];},
                                                //ESC[31m: Set foreground to Red
                                                31 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[1] else tb.basic_colours[9];},
                                                //ESC[32m: Set foreground to Green
                                                32 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[2] else tb.basic_colours[10];},
                                                //ESC[33m: Set foreground to Yellow
                                                33 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[3] else tb.basic_colours[11];},
                                                //ESC[34m: Set foreground to Blue
                                                34 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[4] else tb.basic_colours[12];},
                                                //ESC[35m: Set foreground to Magenta
                                                35 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[5] else tb.basic_colours[13];},
                                                //ESC[36m: Set foreground to Cyan
                                                36 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[6] else tb.basic_colours[14];},
                                                //ESC[37m: Set foreground to White
                                                37 => {self.main_buf.currentForegroundColour = if (!is_bold) tb.basic_colours[7] else tb.basic_colours[15];},
                                                //ESC[38m: Set foreground using xterm-256 or RGB
                                                38 => {sp_fg = 1;},
                                                //ESC[39m: Set foreground to default
                                                39 => {self.main_buf.currentForegroundColour = self.main_buf.foregroundColour;},

                                                //ESC[40m: Set background to Black
                                                40 => {self.main_buf.currentBackgroundColour = tb.basic_colours[0];},
                                                //ESC[41m: Set background to Red
                                                41 => {self.main_buf.currentBackgroundColour = tb.basic_colours[1];},
                                                //ESC[42m: Set background to Green
                                                42 => {self.main_buf.currentBackgroundColour = tb.basic_colours[2];},
                                                //ESC[43m: Set background to Yellow
                                                43 => {self.main_buf.currentBackgroundColour = tb.basic_colours[3];},
                                                //ESC[44m: Set background to Blue
                                                44 => {self.main_buf.currentBackgroundColour = tb.basic_colours[4];},
                                                //ESC[45m: Set background to Magenta
                                                45 => {self.main_buf.currentBackgroundColour = tb.basic_colours[5];},
                                                //ESC[46m: Set background to Cyan
                                                46 => {self.main_buf.currentBackgroundColour = tb.basic_colours[6];},
                                                //ESC[47m: Set background to White
                                                47 => {self.main_buf.currentBackgroundColour = tb.basic_colours[7];},
                                                //ESC[48m: Set background using xterm-256 or RGB
                                                48 => {sp_bg = 1;},
                                                //ESC[49m: Set background to default
                                                49 => {self.main_buf.currentBackgroundColour = self.main_buf.backgroundColour;},

                                                //ESC[90m: Set foreground to Bright Black
                                                90 => {self.main_buf.currentForegroundColour = tb.basic_colours[8];},
                                                //ESC[91m: Set foreground to Bright Red
                                                91 => {self.main_buf.currentForegroundColour = tb.basic_colours[9];},
                                                //ESC[92m: Set foreground to Bright Green
                                                92 => {self.main_buf.currentForegroundColour = tb.basic_colours[10];},
                                                //ESC[93m: Set foreground to Bright Yellow
                                                93 => {self.main_buf.currentForegroundColour = tb.basic_colours[11];},
                                                //ESC[94m: Set foreground to Bright Blue
                                                94 => {self.main_buf.currentForegroundColour = tb.basic_colours[12];},
                                                //ESC[95m: Set foreground to Bright Magenta
                                                95 => {self.main_buf.currentForegroundColour = tb.basic_colours[13];},
                                                //ESC[96m: Set foreground to Bright Cyan
                                                96 => {self.main_buf.currentForegroundColour = tb.basic_colours[14];},
                                                //ESC[97m: Set foreground to Bright White
                                                97 => {self.main_buf.currentForegroundColour = tb.basic_colours[15];},

                                                //ESC[100m: Set background to Bright Black
                                                100 => {self.main_buf.currentBackgroundColour = tb.basic_colours[8];},
                                                //ESC[101m: Set background to Bright Red
                                                101 => {self.main_buf.currentBackgroundColour = tb.basic_colours[9];},
                                                //ESC[102m: Set background to Bright Green
                                                102 => {self.main_buf.currentBackgroundColour = tb.basic_colours[10];},
                                                //ESC[103m: Set background to Bright Yellow
                                                103 => {self.main_buf.currentBackgroundColour = tb.basic_colours[11];},
                                                //ESC[104m: Set background to Bright Blue
                                                104 => {self.main_buf.currentBackgroundColour = tb.basic_colours[12];},
                                                //ESC[105m: Set background to Bright Magenta
                                                105 => {self.main_buf.currentBackgroundColour = tb.basic_colours[13];},
                                                //ESC[106m: Set background to Bright Cyan
                                                106 => {self.main_buf.currentBackgroundColour = tb.basic_colours[14];},
                                                //ESC[107m: Set background to Bright White
                                                107 => {self.main_buf.currentBackgroundColour = tb.basic_colours[15];},
                                    ...
                                    'l' => {
                                        if(args.items.len > 0 and args.items[args.items.len-1] == 1049) self.main_buf = self.alt_buf;
                                        args.clearRetainingCapacity();
                                    },
                                    'h' => {
                                        if(args.items.len > 0 and args.items[args.items.len-1] == 1049) self.main_buf = self.text_buf;
                                        args.clearRetainingCapacity();
                                    },
